//
//  EditProviderProfileViewController.swift
...
class EditProviderProfileViewController: UIViewController {
    ...
    func updateProfileDetails() {
        ...
        UtilityClass.showActivityIndicator()
        let providerParam  = ["Name":txtName.text!,"Email":txtEmail.text!,"Address":txtAddress.text!, "ProviderId" : RDGlobalFunction.getObjectFromUserDefault(iSDefaultKey: RDDataEngineClass.userProfileInfoDef).value(forKey: UserProfileKey.ProviderId)!] as [String : Any]
      
        
        
        ClientProviderAPIManager.sharedInstance.postProviderProfileUpdate(selectedVC: self, providerParameters:providerParam , postProviderDetails: PROVIDERPROFILEUPDATE) { (responseObject, success) in
    ...
    func updateProfileImage() {
        ...
        let providerParam  = ["ProfileImg":convertImageToBase64(image: imgProfile.image!), "ProviderId" : RDGlobalFunction.getObjectFromUserDefault(iSDefaultKey: RDDataEngineClass.userProfileInfoDef).value(forKey: UserProfileKey.ProviderId)!] as [String : Any]
        //
        
        ClientProviderAPIManager.sharedInstance.postProviderProfileImageUpdate(selectedVC: self, providerParameters: providerParam, postProviderImageDetails:PROVIDERPROFILEIMAGEUPDATE) { (responseObject, success) in
...
extension EditProviderProfileViewController : UIImagePickerControllerDelegate,UINavigationControllerDelegate {
...
extension EditProviderProfileViewController : UITextFieldDelegate {
